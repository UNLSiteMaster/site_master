...
CREATE TABLE IF NOT EXISTS `scanned_page_links` (
  ...
  INDEX `scanned_page_links_original_url` (`original_url_hash` ASC),
  ...
  INDEX `scanned_page_url` (`original_url_hash` ASC, `final_url_hash` ASC),
...
CREATE TABLE IF NOT EXISTS `site_scan_metric_history` (
  ...
  `date_created` DATETIME NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_site_scan_metric_history_sites1_idx` (`sites_id` ASC),
  INDEX `fk_site_scan_metric_history_metrics1_idx` (`metrics_id` ASC),
  CONSTRAINT `fk_site_scan_metric_history_sites1`
    FOREIGN KEY (`sites_id`)
    REFERENCES `sites` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
