...
function performPluginUpdate(\SiteMaster\Core\Plugin\PluginInterface $plugin)
{
    static $checked;
...
foreach ($internalPlugins as $name=>$options) {
    performPluginUpdate($pluginManager->getPluginInfo($name));
}
