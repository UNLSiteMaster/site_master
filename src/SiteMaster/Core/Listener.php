...
class Listener extends PluginListener
{
    public function onRoutesCompile(RoutesCompile $event)
    {
        ...
        $event->addRoute('/^sites\/(?P<site_id>(\d*))\/reviews(\/(?P<reviews_id>(\d*)))?\/edit\/$/', 'SiteMaster\Core\Auditor\Review\EditForm');
        $event->addRoute('/^sites\/(?P<site_id>(\d*))\/reviews(\/(?P<reviews_id>(\d*)))?\/$/', 'SiteMaster\Core\Auditor\Review\ViewAllForSite');
